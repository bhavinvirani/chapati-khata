...
- [x] `installNotifyHook` POSTs `{"action":"install-hook","url":…}` to the deployed function with the `x-khata-hook` header, triggered by any setting marked `installHook`. A warning, never fatal, and a `401` is named specifically: it means the deployed function still holds the old secret and needs redeploying. The README carries the equivalent `select public.install_notify_hook(…)`.
